...
namespace K4os.Compression.LZ4.Streams
{
	...
	public class LZ4DecoderStream: Stream, IDisposable
	{
        ...
        public override async Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        {
			...
			while (count > 0)
			{
				if (_decoded <= 0 && (_decoded = await ReadBlockAsync(cancellationToken)) == 0)
					break;
		...
		private async ValueTask<int> ReadBlockAsync(CancellationToken cancellationToken)
		{
			FlushPeek();

			var blockLength =(int) await Peek32Async(cancellationToken);
			if (blockLength == 0)
			{
				if (_frameInfo.ContentChecksum)
					await Peek32Async(cancellationToken);
				CloseFrame();
				return 0;
			}
			...
			await PeekNAsync(_buffer, 0, blockLength,cancellationToken);
		...
		private async Task<int> PeekNAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken,bool optional = false)
		{
			var index = 0;
			while (count > 0)
			{
				var read = await _inner.ReadAsync(buffer, index + offset, count, cancellationToken);
				if (read == 0)
				{
					if (index == 0 && optional)
						return 0;
		...
		private async ValueTask<bool> PeekNAsync(int count, CancellationToken cancellationToken,bool optional = false)
		{
			if (count == 0) return true;

			var read = await PeekNAsync(_buffer16, _index16, count, cancellationToken, optional);
			_index16 += read;
			return read > 0;
		}
		...
		private async ValueTask<uint> Peek32Async(CancellationToken cancellationToken)
		{
			await PeekNAsync(sizeof(uint), cancellationToken: cancellationToken);
			return BitConverter.ToUInt32(_buffer16, _index16 - sizeof(uint));
		}
