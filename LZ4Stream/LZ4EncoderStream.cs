...
namespace K4os.Compression.LZ4.AsyncStreams
{
	...
	public class LZ4EncoderStream: Stream, IDisposable
	{
		...
		private ValueTask WriteBlockAsync(int length, EncoderAction action, CancellationToken cancellationToken)
		{
			switch (action)
			{
				case EncoderAction.Copied:
					return WriteBlockAsync(length, false, cancellationToken);					
				case EncoderAction.Encoded:
					return WriteBlockAsync(length, true, cancellationToken);					
			}
		...
		private async ValueTask WriteBlockAsync(int length, bool compressed,CancellationToken cancellationToken)
		{
			if (length <= 0)
				return;
